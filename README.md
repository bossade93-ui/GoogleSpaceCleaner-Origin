...
- Aucune Migration Room réelle n'existe encore (`fallbackToDestructiveMigration`) — acceptable tant qu'aucune version n'est publiée avec des utilisateurs réels ; **à remplacer avant toute publication**. Le schéma est désormais exporté à chaque version (`room.schemaLocation`, voir `core/core-data/build.gradle.kts`) pour pouvoir écrire de vraies `Migration()` le moment venu.
